...
def evaluation(args, model, data_loader, tokenizer, device, config):
    ...
    for cls_idx in range(logits_multicls.shape[1]):
        total_multicls = TP_all_multicls[cls_idx] + TN_all_multicls[cls_idx] + FP_all_multicls[cls_idx] + FN_all_multicls[cls_idx]
        mACC_multicls[cls_idx] = (TP_all_multicls[cls_idx] + TN_all_multicls[cls_idx]) / total_multicls
    ...
    return AUC_cls, ACC_cls, EER_cls, \
           MAP.item(), OP, OR, OF1, CP, CR, CF1, mACC_multicls.mean(), OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k, \
...
def main_worker(gpu, args, config):
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        MAP, OP, OR, OF1, CP, CR, CF1, mACC, OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k, \
        IOU_score, IOU_ACC_50, IOU_ACC_75, IOU_ACC_95, \
        ACC_tok, Precision_tok, Recall_tok, F1_tok \
        ...
        if utils.is_main_process(): 
            ...
            if (epoch % args.model_save_epoch == 0 and epoch!=0):
                checkpoint_path = os.path.join(log_dir, 'checkpoint_%02d.pth'%epoch)
                torch.save(save_obj, checkpoint_path)
                prev_epoch = epoch - args.model_save_epoch
                if prev_epoch > 0:
                    prev_checkpoint_path = os.path.join(log_dir, 'checkpoint_%02d.pth'%prev_epoch)
