...
class HAMMER(nn.Module):
    def __init__(self, 
        ...
        if init_deit:
            local_deit = config.get('deit_checkpoint', 'local_models/vit-weight/checkpoints/checkpoints/deit_base_patch16_224-b5f2ef4d.pth')
            if local_deit and os.path.exists(local_deit):
                checkpoint = torch.load(local_deit, map_location="cpu")
            else:
    ...
    def _dequeue_and_enqueue(self, image_feat, text_feat):
        ...
        # Replace the keys at ptr. The original code required queue_size to be
        # divisible by batch_size; this ring-buffer update supports larger
        # single-GPU batches such as 48 without changing the queue size.
        if end <= self.queue_size:
            self.image_queue[:, ptr:end] = image_feats.T
            self.text_queue[:, ptr:end] = text_feats.T
        else:
            first = self.queue_size - ptr
            self.image_queue[:, ptr:] = image_feats[:first].T
            self.text_queue[:, ptr:] = text_feats[:first].T
